...
export class CouponPage extends React.Component<Props, { status: Status }>{
    ...
    loadData(pageIndex: number, status: string) {
        return this.props.shopping.myCoupons(pageIndex, status);
    }
    componentDidUpdate() {
        this.dataList.reset();
            ...
            <div>
                <header>
                    ...
                    <Tabs className="tabs" defaultActiveIndex={defaultIndex} onItemClick={(index) => this.activeItem(index)}
                        ...
                        {statuses.map(o => (
                            <span key={o}>{this.props.shopping.couponStatusText(o)}</span>
                        ))}
                    </Tabs>
                </header>
                    ...
                    <DataList ref={o => this.dataList = o}
                        ...
                        emptyItem={
                            <div className="norecords">
                                ...
                                <h4 className="text">暂无{this.props.shopping.couponStatusText(status)}的优惠券</h4>
                            </div>
                        } />
                </section>
            </div>
