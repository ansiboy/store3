...
export class ShoppingCartPage extends React.Component<Props, ShoppingCartState>{
    ...
    private buy() {
        ...
        let result = this.props.shop.createOrder(productIds, quantities)
            .then((order) => {
...
export default function (page: chitu.Page, hideMenu?: boolean) {
    let shop = page.createService(ShoppingService);
    hideMenu = hideMenu == null ? false : hideMenu;
    let props = {
        shop,
        hideMenu,
        pageName: page.name,
    };
    ReactDOM.render(<ShoppingCartPage {...props} />, page.element);
}
